...
class SmelterRefuelerBot(GobSelectCallback, AreaSelectCallback, Window):
    state = State.WAIT
    gui = None
    ...
    def __init__(self, coord, title):
        ...
        self.add(Label("Smelters selected: "), Coord(15, 15))
        self.lbl_smelters = Label("0")
        self.add(self.lbl_smelters, Coord(120,15))
    ...
    def gobselect(self, gob):
        ...
        if(res != None):
            ...
            if(res.name.startswith("gfx/terobjs/smelter")):
                self.smelters.append(gob)
                self.lbl_smelters.settext("{0}".format(len(self.smelters)))
        ...
        while self.state != State.TERM:
            if self.state == State.RUN:
                for smelter in self.smelters:
                    HavenPanel.lui.cons.out.println("Filling {0}".format(smelter))
                    availableFuelCoal = self.gui.maininv.getItemPartialCount("Coal")
                    availableFuelBlock = self.gui.maininv.getItemPartialCount("Block")
                    availableFuelBranch = self.gui.maininv.getItemPartialCount("Branch")
                    ...
                    self.gui.map.pfRightClick(smelter, -1, 3, 0, None)
                    self.gui.map.pfthread.join()
                    cwnd = self.gui.waitfForWnd("Ore Smelter", 1500)
                    if cwnd == None:
                    ...
                    for step in range(fueltoload):
                        ...
                        while(self.gui.hand.isEmpty() and self.state == State.RUN):
                            ...
                            sleep(1)
                        self.gui.map.wdgmsg("itemact", Coord.z, smelter.rc.floor(OCache.posres), 0, 0, int(smelter.id), smelter.rc.floor(OCache.posres), 0, -1)
...
# Our Automation
with SmelterRefuelerBot(Coord(270,180),"Smelter Refueler Bot") as bot:
    bot.run()
